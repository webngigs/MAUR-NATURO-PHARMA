...
//selected manufacturer product
	...
	public function proforma_data($product_id)
	{
		$this->db->select('a.*,b.*,c.customer_name');
		$this->db->from('invoice a');
		$this->db->where('a.type_of_invoice', 2);
		$this->db->join('invoice_details b','b.invoice_id = a.invoice_id');
		$this->db->join('customer_information c','c.customer_id = a.customer_id');
		$this->db->where('b.product_id',$product_id);
		$this->db->order_by('a.invoice_id','asc');
	...
	public function stockrequest_data($product_id){
	    $this->db->select('a.*, b.*, c.mr_name');  
	    $this->db->from('stockrequests a');
	    $this->db->join('stockrequests_details b', 'b.stockrequests_id = a.stockrequests_id');
	    $this->db->join('mr_information c', 'c.user_id = a.mr_id');
	    $this->db->where('b.product_id', $product_id);
	    $this->db->order_by('a.stockrequests_id','asc');
		$query = $this->db->get();
