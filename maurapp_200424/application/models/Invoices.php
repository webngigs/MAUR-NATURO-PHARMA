...
class Invoices extends CI_Model {
	...
	public function total_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_demand_request))    return $query[0]->total_demand_request;
	        else return 0;   
	    }
	        ...
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('request_by', 'mr');
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_demand_request))    return $query[0]->total_demand_request;
	        else return 0;   
	    }    
	}
	public function total_approve_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
	...
	public function total_pending_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
	...
	public function total_cust_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
	        ...
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('request_by', 'customer');
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_demand_request))    return $query[0]->total_demand_request;
	        else return 0;   
	    }    
	}
	public function total_cust_approve_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
	...
	public function total_cust_pending_demand_request(){
	    if($this->session->userdata('user_type') == 3){
	        $this->db->select('COUNT(1) AS total_demand_request');
    		$this->db->from('demandrequests');
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
	...
	public function total_sales_amount(){
        if($this->session->userdata('user_type') == 3){
    		...
    		$this->db->where('mr_id', $this->session->userdata('user_id'));
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_sales))    return $query[0]->total_sales;
	        else return 0;   
	    }
	    else return 0;    
	}
	
	public function total_proforma_invoice_amount(){
        if($this->session->userdata('user_type') == 3){
	        $this->db->select('SUM(total_amount) AS total_sales');
    		$this->db->from('invoice');
	...
	public function mr_total_sales(){
	    if($this->session->userdata('user_type') == 1){
	        $this->db->select('SUM(total_amount) AS total_sales');
    		$this->db->from('invoice');
    		$this->db->where('type_of_invoice', 1);
    		$this->db->where('mr_id > ', 0);
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_sales))    return $query[0]->total_sales;
	        else return 0;   
	    }
	...
	public function mr_total_proforma_invoice(){
	    if($this->session->userdata('user_type') == 1){
	        $this->db->select('SUM(total_amount) AS total_sales');
    		$this->db->from('invoice');
    		$this->db->where('type_of_invoice', 2);
	...
	public function customer_total_purchase(){
	    if($this->session->userdata('user_type') == 1){
    		...
    		$query = $this->db->get()->result();
    		if(isset($query[0]->total_purchase))    return $query[0]->total_purchase;
	        else return 0;    
	    }
	    else if($this->session->userdata('user_type') == 4){
	        $customer_id = 0;
	...
	public function customer_total_proforma_invoice(){
	    if($this->session->userdata('user_type') == 1){
	        $this->db->select('SUM(total_amount) AS total_purchase');
    		$this->db->from('invoice');
    		$this->db->where('type_of_invoice', 2);
	...
	public function __getTotalAmountOfInvoices(){
		$customer_id 			=	0;
		$totalInvoiceAmount 	=	0;
		$ClaimedInvoiceAmount 	=	0;
		$TobeClaimInvoiceAmount = 	0;
		...
		$TobeClaimInvoiceAmount =	($totalInvoiceAmount/100) - $ClaimedInvoiceAmount;

		$data = array();
		$data['totalInvoiceAmount'] 	= 	$totalInvoiceAmount;
		$data['ClaimedInvoiceAmount'] 	= 	$ClaimedInvoiceAmount;
		$data['TobeClaimInvoiceAmount'] =	$TobeClaimInvoiceAmount;
		return $data;
	}
	...
	public function count_invoice()
	{
	    if($this->session->userdata('user_type') == 4){
	        ...
	        return $this->db->where('type_of_invoice', 1)->where('customer_id', $customer_id)->from("invoice")->count_all_results();
	    }
	    else if($this->session->userdata('user_type') == 3){
	        return $this->db->where('type_of_invoice', 1)->where('mr_id', $this->session->userdata('user_id'))->from("invoice")->count_all_results();
	    }
		else return $this->db->where('type_of_invoice', 1)->from("invoice")->count_all_results();
	}
	
	public function count_proforma()
	{
	    if($this->session->userdata('user_type') == 4){
	        $customer_id = 0;
			$this->db->select('customer_id');
	        ...
	        return $this->db->where('type_of_invoice', 2)->where('customer_id', $customer_id)->from("invoice")->count_all_results();
	    }
	    else if($this->session->userdata('user_type') == 3){
	        return $this->db->where('type_of_invoice', 2)->where('mr_id', $this->session->userdata('user_id'))->from("invoice")->count_all_results();
	    }
		else return $this->db->where('type_of_invoice', 2)->from("invoice")->count_all_results();
	}
	...
	public function getInvoiceList($postData=null){
		...
		if($this->session->userdata('user_type') == 4){
			...
			$this->db->where('a.mr_id', $this->session->userdata('user_id'));
		}
		...
		foreach($records as $record ){
			...
			$button .=' <a href="'.$base_url.'Cinvoice/invoice_inserted_data_pdf/'.$record->invoice_id.'" target="_blank" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('invoice').'"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>';
            ...
            $data[] = array( 
                ...
                'total_amount'     =>	$record->total_amount.' INR',
                'button'           =>	$button                
            ); 
            $sl++;
        }
	...
	public function getProformaInvoiceList($postData=null){
		$this->load->library('occational');
		$response = array();
		$fromdate = $this->input->post('fromdate');
		$todate   = $this->input->post('todate');
		...
		foreach($records as $record ){
			...
			if($this->session->userdata('user_type') == 1){
    			if(isset($record->convert_into_invoice) && $record->convert_into_invoice==0) {
    				$button .=  '<p><button  class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="Click to Convert Proform to Invoice" onclick="convertIntoInvoice('.$record->invoice_id.');">Convert Invoice</button></p>';
    			    $button .=  ' <a href="'.$base_url.'Cproformainvoice/invoice_inserted_data/'.$record->invoice_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('proforma_invoice').'"><i class="fa fa-window-restore" aria-hidden="true"></i></a>';
    			    $button .=  ' <a href="'.$base_url.'Cproformainvoice/proforma_invoice_update_form/'.$record->invoice_id.'" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="left" title="'. display('edit').'"><i class="fa fa-edit"></i></a>';
    			    $button .=  ' <a href="'.$base_url.'Cproformainvoice/proforma_invoice_delete/'.$record->invoice_id.'" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="left" title="'. display('delete').'"  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
    			}
    			else{
    			    $button .=' <a href="'.$base_url.'Cproformainvoice/invoice_inserted_data/'.$record->invoice_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('proforma_invoice').'"><i class="fa fa-window-restore" aria-hidden="true"></i></a>';
    			}
			...
			$button .=' <a href="'.$base_url.'Cproformainvoice/invoice_inserted_data_pdf/'.$record->invoice_id.'" target="_blank" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('proforma_invoice').'"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>';
	...
	public function pos_invoice_setup($product_id){
		...
		if ($product_information != null) {
			...
			$data2 = (object)array(
				...
				'tax0' 		=> $product_information->tax0, 
				'box_size' 		=> $product_information->box_size, 
				'HSNcode' 		=> $product_information->HSNcode, 
	...
	public function invoice_entry()
	{
		...
		if($this->session->userdata('user_type')==3)	{
		    ...
		    $created_by_mr =  $this->session->userdata('user_id');
		}
	...
	public function proforma_invoice_entry(){
		$tablecolumn 		= 	$this->db->list_fields('tax_collection');
        $num_column 		= 	count($tablecolumn)-4;
		$invoice_id 		= 	$this->generator(10);
		$invoice_id 		= 	strtoupper($invoice_id);
		...
		if ($product_id == null) {
			$this->session->set_userdata(array('error_message'=>display('please_select_product')));
			redirect('Cproformainvoice/pos_invoice');
		}

		if (($this->input->post('customer_name_others') == null) && ($this->input->post('customer_id') == null ) && ($this->input->post('customer_name') == null )) {
			$this->session->set_userdata(array('error_message'=>display('please_select_customer')));
			redirect(base_url().'Cproformainvoice');
		}
		...
		$proforma_no = $this->proforma_number_generator();
		...
		$cc = array(
			...
			'VDate'          =>  $createdate,
			'COAID'          =>  1020101,
			'Narration'      =>  'Cash in Hand For Proforma No'.$invoice_id,
			'Debit'          =>  $this->input->post('paid_amount'),
			'Credit'         =>  0,
			'IsPosted'       =>  1,
			'CreateBy'       =>  $createby,
		...
		$bankc = array(
			...
			'VDate'          =>  $createdate,
			'COAID'          =>  $bankcoaid,
			'Narration'      =>  'Paid amount for Proforma No '.$invoice_id,
			'Debit'          =>  $this->input->post('paid_amount'),
			'Credit'         =>  0,
			'IsPosted'       =>  1,
			'CreateBy'       =>  $createby,
		...
		$inv_credit = array(
			...
			'VDate'          =>  $createdate,
			'COAID'          =>  10107,
			'Narration'      =>  'Inventory credit For Proforma No'.$invoice_id,
			'Debit'          =>  0,
			'Credit'         =>  $sumval,//purchase price asbe
			'IsPosted'       => 1,
			'CreateBy'       => $createby,
        ...
        $cosdr = array(
			...
			'VDate'          =>  $createdate,
			'COAID'          =>  $customer_headcode,
			'Narration'      =>  'Customer debit For Proforma No'.$invoice_id,
			'Debit'          =>  $this->input->post('n_total')-(!empty($this->input->post('previous'))?$this->input->post('previous'):0),
			'Credit'         =>  0,
			'IsPosted'       => 1,
			'CreateBy'       => $createby,
        ...
        $cuscredit = array(
            ...
            'VDate'          =>  $createdate,
            'COAID'          =>  $customer_headcode,
            'Narration'      =>  'Customer credit for Paid Amount For Proforma No'.$invoice_id,
            'Debit'          =>  0,
            'Credit'         =>  $this->input->post('paid_amount'),
            'IsPosted'       => 1,
            'CreateBy'       => $createby,
		...
		for ($i=0, $n=count($quantity); $i < $n; $i++) {
			...
			$product_information = $this->db->select('*')->from('product_information')->where('product_id', $product_id)->get()->row(); 
			...
			$data1 = array(
				...
				'tax'           		=>	$tax,
				'tax0'                  =>  $product_information->tax0,
				'item_rate'             =>  $product_information->item_rate,
				'paid_amount'           =>	$this->input->post('paid_amount'),
				'due_amount'           	=>	$this->input->post('due_amount'),
				'manufacturer_rate'     =>	$manufacturer_rate[0]['manufacturer_price'],
				'total_price'           =>	$total_price,
	   	...
	   	$currency_details = $this->Web_settings->retrieve_setting_editdata();
		$message = 'Mr/Mrs.'.$customerinfo->customer_name.', '.'You have purchase  '.$this->input->post('grand_total_price').' '. $currency_details[0]['currency'].' You have paid .'.$this->input->post('paid_amount').' '. $currency_details[0]['currency'];
       
        //$this->send_sms($customerinfo->customer_mobile, $message);
        
        $this->load->library('email');
        
        $this->load->library('lproformainvoice');
		$html_data = $this->lproformainvoice->invoice_html_data_pdf($invoice_id);
		$this->load->library('pdfgenerator');
        $this->pdfgenerator->generate($html_data, $invoice_id.'.pdf', FALSE, 'A4', "landscape");
		...
		if($query->num_rows() > 0) {
			...
			$message .= '<br/>Please Check Proforma Invoice ID#'.$proforma_no;
		    $message .= '<br/>Please <a href="https://app.maurnaturo.com/Cproformainvoice/invoice_inserted_data/'.$invoice_id.'">Click Here</a>';
            ...
            $this->email->subject('Proforma Invoice ID#'.$proforma_no);
            $this->email->message($message);
            $this->email->attach($_SERVER["DOCUMENT_ROOT"].'/'.$invoice_id.'.pdf');
    ...
    public function send_sms($phone=null,$msg=null){
		...
		if($config_data->isinvoice == 0)	return true;
		else{
			$recipients=$phone;
	...
	{
		$this->db->select('a.*, c.*, a.total_tax, b.customer_name, c.batch_id, c.tax as t_p_tax, c.product_id, d.product_name, d.box_size, d.tax0, d.HSNcode, d.product_model, d.tax, d.unit');
		$this->db->from('invoice a');
		$this->db->join('customer_information b','b.customer_id = a.customer_id');
	...
	public function retrieve_invoice_html_data($invoice_id)
	{
		$this->db->select('a.total_tax,
						...
						d.product_model,
						d.item_rate,
						(SELECT expeire_date FROM product_purchase_details WHERE product_purchase_details.product_id = d.product_id LIMIT 1) AS expeire_date');
		$this->db->from('invoice a');
		$this->db->join('invoice_details c','c.invoice_id = a.invoice_id');
		$this->db->join('customer_information b','b.customer_id = a.customer_id');
		$this->db->join('product_information d','d.product_id = c.product_id');
	...
	public function proforma_number_generator()
	{
		$this->db->select_max('proforma', 'proforma_no');
		$query = $this->db->get('invoice');	
		$result = $query->result_array();	
		$proforma_no = $result[0]['proforma_no'];
		if($proforma_no !='')	$proforma_no = $proforma_no + 1;
		else	$proforma_no = 1;
		return $proforma_no;		
	}
	...
	public function get_total_product_batch($batch_id){
		...
		if($this->session->userdata('user_type')==4)	{
			$this->db->select('SUM(b.quantity) as total_sale');
			$this->db->from('invoice_details b');
			$this->db->where('b.type_of_invoice', 2);
			...
			$this->db->select('SUM(c.quantity) AS quantity, d.item_rate');
			...
			$this->db->where('c.product_id', $total_purchase->product_id);
			$totalRecords = $this->db->get()->row();
			
			$item_rate = $totalRecords->item_rate;
			...
			$available_quantity = ($totalRecords->quantity - $total_sale->total_sale);
		} 
		else{
			$this->db->select('a.expeire_date, SUM(a.quantity) as total_purchase, a.tax0, a.product_id');
			$this->db->from('product_purchase_details a');
			$this->db->where('a.batch_id',$batch_id);
			$this->db->order_by('a.id','desc');
			$total_purchase = $this->db->get()->row();
			...
			$this->db->join('stockrequests_details c','c.stockrequests_id = a.stockrequests_id');
			$this->db->join('product_information d','d.product_id = c.product_id');
			$this->db->where('a.createby', $createby);
			$this->db->where('c.batch_id', $batch_id);
			...
			$this->db->where('b.batch_id', $batch_id);
			$total_sale = $this->db->get()->row();
			$available_quantity = ($total_purchase->total_purchase - $stockTransferUnits);
		}
    ...
    public function autocompletproductdata($product_name){
        if($this->session->userdata('user_type') == 3)	{
		    $query=$this->db->select('*')
				...
				->join('stockrequests_details AS sd', 'sd.product_id = pi.product_id')
				->join('stockrequests AS s', 's.stockrequests_id = sd.stockrequests_id')
				->where('s.mr_id', $this->session->userdata('user_id'))
                ->where_in('s.request_by', ['mr','admin'])
				->where('s.status', 4)
				->like('pi.product_name', $product_name, 'after')
            ...
            $who_added = 1;
            $customer_id = $this->input->post('customer_id');
            $this->db->select('user_id, who_added');
			$this->db->from('customer_information');
			$this->db->where('customer_id', $customer_id);
			$query = $this->db->get();
			if($query->num_rows() > 0) {
				$dts = $query->result_array();
				if($dts[0]['user_id'] == 1) $mr_id = 0;
				else $mr_id = $dts[0]['user_id'];
				$who_added = $dts[0]['who_added'];
			}
	...
	public function convertProformaToInvoice($pid){
		$this->db->select('*');
		$this->db->from('invoice');
		$this->db->where('invoice_id', $pid);			
		$proformaData = $this->db->get()->result();	

		if(isset($proformaData[0]->id) && $proformaData[0]->id>0){
			$invoice_id 	=	$this->generator(10);
			$invoice_id 	= 	strtoupper($invoice_id);
			//$transection_id	=	$this->auth->generator(15);
			//$createby		=	$this->session->userdata('user_id');
			...
			$datainv	=	array(
				'type_of_invoice'	=>	1,
				'invoice_id'		=>	$invoice_id,
				'customer_id'		=>	$proformaData[0]->customer_id,
				'mr_id'				=>	$proformaData[0]->mr_id,
				'date'				=>	date('Y-m-d'),
				'total_amount'		=>	$proformaData[0]->total_amount,
				'total_tax'			=>	$proformaData[0]->total_tax,
				'invoice'			=>	$invoice_no,
				'invoice_details'   =>  $proformaData[0]->invoice_details,
				'total_discount' 	=> 	$proformaData[0]->total_discount,
				'invoice_discount' 	=> 	$proformaData[0]->invoice_discount,
				'prevous_due'       =>  $proformaData[0]->prevous_due,
				'sales_by'          =>  $proformaData[0]->sales_by,
				'status'			=>	$proformaData[0]->status,
				'payment_type'      =>  $proformaData[0]->payment_type,
				'bank_id'           =>  (!empty($proformaData[0]->bank_id)?$proformaData[0]->bank_id:null),
			);
			$this->db->insert('invoice', $datainv);

			$this->db->select('*');
			$this->db->from('tax_collection');
			$this->db->where('relation_id', $proformaData[0]->invoice_id);			
			$TaxCollection	=	$this->db->get()->result();
			if(isset($TaxCollection) && count($TaxCollection)>0){
				foreach($TaxCollection as $txKey=>$txVal){
					$taxdata = array();
					$taxdata['date']        =	date('Y-m-d');
					$taxdata['customer_id']	= 	$txVal->customer_id;
					$taxdata['relation_id'] = 	$invoice_id;
					$taxdata['tax0'] 		= 	$txVal->tax0;
					$taxdata['tax1'] 		= 	$txVal->tax1;
					$taxdata['tax2'] 		= 	$txVal->tax2;
					$this->db->insert('tax_collection', $taxdata);	
				}	
			}
			...
			$this->db->select('*');
			$this->db->from('invoice_details');
			$this->db->where('invoice_id', $proformaData[0]->invoice_id);			
			$proformaDetails = $this->db->get()->result();

			if(isset($proformaDetails) && count($proformaDetails)>0){
				foreach($proformaDetails as $pdKey=>$pdVal){
					$invoice_details = array(
					    'type_of_invoice'	=>	1,
						'invoice_details_id'	=>	$this->generator(15),
						'invoice_id'			=>	$invoice_id,
						'product_id'			=>	$pdVal->product_id,
						'batch_id'              =>  $pdVal->batch_id,
						'quantity'				=>	$pdVal->quantity,
						'rate'					=>	$pdVal->rate,
						'discount'           	=>	$pdVal->discount,
						'tax'           		=>	$pdVal->tax,
						'tax0'           		=>	$pdVal->tax0,
						'item_rate'             =>	$pdVal->item_rate,
						'paid_amount'           =>	$pdVal->paid_amount,
						'due_amount'           	=>	$pdVal->due_amount,
						'manufacturer_rate'     =>	$pdVal->manufacturer_rate,
						'total_price'           =>	$pdVal->total_price,
						'status'				=>	1,
						'created_by_mr'         =>  $pdVal->created_by_mr
					);
			...
			$this->db->where('invoice_id', $pid);
			$this->db->update('invoice', array('convert_into_invoice' => $invoice_id));
    		...
    		$html_data = $this->linvoice->invoice_html_data_pdf($invoice_id);
    		$this->load->library('pdfgenerator');
            $this->pdfgenerator->generate($html_data, $invoice_id.'.pdf', FALSE, 'A4', "landscape");
			...
			$this->db->select('user_id, customer_name, customer_email');
			$this->db->from('customer_information');
			$this->db->where('customer_id', $proformaData[0]->customer_id);
			$query = $this->db->get();
			if($query->num_rows() > 0) {
				$dts = $query->result_array();
				
				$message = 'Hi '.$dts[0]['customer_name'].',<br/>';
    			$message .= '<br/>Please Check Invoice ID#'.$invoice_no;
    		    $message .= '<br/>Please <a href="https://app.maurnaturo.com/Cinvoice/invoice_inserted_data/'.$invoice_id.'">Click Here</a>';
    		    $message .= '<br/><br/>--<br/>Thanks and Regards,<br/>Maurnaturo Team';
    		    
    		    $this->email->from('maurservicesjr@gmail.com', 'Maurnaturo Team');
                $this->email->to($dts[0]['customer_email']);
                $this->email->subject('Invoice ID#'.$invoice_no);
                $this->email->message($message);
                $this->email->attach($_SERVER["DOCUMENT_ROOT"].'/'.$invoice_id.'.pdf');
                $this->email->set_mailtype('html');
                $this->email->send();
			...
			$this->db->select('user_id, mr_email, mr_name');
			$this->db->from('mr_information');
			$this->db->where('user_id', $proformaData[0]->mr_id);
			$querys = $this->db->get();
			if($querys->num_rows() > 0) {
			    $dtss = $querys->result_array();
			    $message = 'Hi '.$dtss[0]['mr_name'].',<br/>';
