...
class Lproduct {
	...
	*/
	...
	public function product_edit_data($product_id)
	{
		...
		$data['item_rate'] 		    = $product_detail[0]['item_rate'];
		...
		$data['HSNCode'] 		    = $product_detail[0]['HSNCode'];
	...
	public function product_details($product_id)
	{
		...
		$salesProformaData = $CI->Products->proforma_data($product_id);	

		$totalProformaSales     =   0;		
		$totaProformaSalesAmt   =   0;		
		if(!empty($salesProformaData)){	
			foreach($salesProformaData as $k=>$v){
				$salesProformaData[$k]['final_date'] = $CI->occational->dateConvert($salesProformaData[$k]['date']);
				$totalProformaSales = ($totalProformaSales + $salesProformaData[$k]['quantity']);
				$totaProformaSalesAmt = ($totaProformaSalesAmt + $salesProformaData[$k]['total_price']);
			}
		}
		
		$totalAdminStock    = 0;
		$totalStockTransfer = 0;
		$stockRequestData = $CI->Products->stockrequest_data($product_id);
		if(!empty($stockRequestData)){	
			foreach($stockRequestData as $k=>$v){
			    $totalStockTransfer = ($totalStockTransfer + $stockRequestData[$k]['quantity']);
				$stockRequestData[$k]['final_date'] = $CI->occational->dateConvert($stockRequestData[$k]['date']);
			}
		}
		
		$totalAdminStock = $totalPurchase - $totalStockTransfer;
		...
		$data = array(
				...
				'salesProformaTotalAmount'		=> number_format($totaProformaSalesAmt, 2, '.', ','),
				'total_purchase'		=> $totalPurchase,
				'total_sales'			=> $totalSales,
				'total_proforma_sales'			=> $totalProformaSales,
				'purchaseData'			=> $purchaseData,
				'salesData'				=> $salesData,
				'salesProformaData'     => $salesProformaData,
				'stockRequestData'      => $stockRequestData,
				'stock'					=> $stock,
				'product_statement' 	=> 'Cproduct/product_sales_manufacturer_rate/'.$product_id,
				'currency' 				=> $currency_details[0]['currency'],
				'position' 				=> $currency_details[0]['currency_position'],
				'totalStockTransfer'    => $totalStockTransfer,
