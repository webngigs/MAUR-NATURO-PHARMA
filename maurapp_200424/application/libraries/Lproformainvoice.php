...
class Lproformainvoice {
    ...
    public function invoice_list() {
        ...
        $invoiceList = $CI->parser->parse('proformainvoice/invoice', $data, true);
        return $invoiceList;
    }
	...
	public function pos_invoice_add_form()
	{
	 ...
	 $taxfield = $CI->db->select('tax_name,default_value')
		...
		$invoiceForm = $CI->parser->parse('proformainvoice/add_pos_invoice_form',$data,true);
		return $invoiceForm;
	}
	...
	public function invoice_add_form()
	{
		...
		$taxfield = $CI->db->select('tax_name, default_value')
            ...
            ->result_array();
		$data = array(
			'title'         => display('add_proforma_invoice'),
			'customer_name' => '',//$customer_details[0]['customer_name'],
			'customer_id' 	=> '',//$customer_details[0]['customer_id'],
			'discount_type' => $currency_details[0]['discount_type'],
			'taxes'         => $taxfield,
		...
		$invoiceForm = $CI->parser->parse('proformainvoice/add_invoice_form', $data, true);
		return $invoiceForm;
	}
	//Insert invoice
...
public function invoice_edit_data($invoice_id)
	{
		...
		$chapterList = $CI->parser->parse('proformainvoice/edit_invoice_form',$data,true);
		return $chapterList;
	}
	...
	public function invoice_html_data($invoice_id)
	{
		...
		$data=array(
			...
			'invoice_id'		=>	$invoice_detail[0]['invoice_id'],
			'proforma_no'		=>	$invoice_detail[0]['proforma'],
			'convert_into_invoice'	=>	$invoice_detail[0]['convert_into_invoice'],
			...
			'pancard'			=>	$invoice_detail[0]['pancard'],
			'rvc_no'			=>	$invoice_detail[0]['rvc_no'],
		...
		$chapterList = $CI->parser->parse('proformainvoice/invoice_html',$data,true);
		return $chapterList;
	}
	
	
	public function invoice_html_data_pdf($invoice_id)
	{
		$CI =& get_instance();
		$CI->load->model('Invoices');
		$CI->load->model('Web_settings');
		...
		$chapterList = $CI->parser->parse('proformainvoice/invoice_html_pdf',$data,true);
		return $chapterList;
	}
	 ...
	 public function invoice_html_data_manual($invoice_id) {
		...
		$chapterList = $CI->parser->parse('proformainvoice/invoice_html_manual',$data,true);
		return $chapterList;
	 }
	...
	public function pos_invoice_html_data($invoice_id)
	{
		...
		$chapterList = $CI->parser->parse('proformainvoice/pos_invoice_html',$data,true);
		return $chapterList;
	}
	...
	public function pos_invoice_html_data_manual($invoice_id) {
		...
		$chapterList = $CI->parser->parse('proformainvoice/pos_invoice_html_direct',$data,true);
		return $chapterList;
