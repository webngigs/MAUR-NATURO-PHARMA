...
class Cproformainvoice extends CI_Controller {
	...
	public function index()
	{
		...
		$CI->load->library('lproformainvoice');
		$content = $CI->lproformainvoice->invoice_add_form();
	...
	public function insert_invoice()
	{
		...
		$invoice_id = $CI->Invoices->proforma_invoice_entry();
		$this->session->set_userdata(array('message'=>display('successfully_added')));
		//$this->invoice_inserted_data($invoice_id);
		redirect("Cproformainvoice/invoice_inserted_data/".$invoice_id);
	}
	 ...
	 public function manual_sales_insert(){
        ...
        if(!empty($invoice_id)){
            ...
            $data['details'] = $this->load->view('proformainvoice/invoice_html', $data, true);
    ...
    public function invoice_inserted_data_manual() {
        ...
        $content = $CI->lproformainvoice->invoice_html_data_manual($invoice_id);
        $this->template->full_admin_html_view($content);
    }
    public function pos_invoice_inserted_data_manual() {
       $CI = & get_instance();
        ...
        $CI->load->library('lproformainvoice');
        $invoice_id = $this->input->post('invoice_id');
        $content = $CI->lproformainvoice->pos_invoice_html_data_manual($invoice_id);
        $this->template->full_admin_html_view($content);
    }
	...
	{	
		...
		$content = $CI->lproformainvoice->invoice_edit_data($invoice_id);
		$this->template->full_admin_html_view($content);
	}
	
	public function proforma_invoice_update_form($invoice_id)
	{	
		$CI =& get_instance();
		$CI->auth->check_admin_auth();
		$CI->load->library('lproformainvoice');
	...
	public function search_inovoice_item()
	{
        ...
        $content = $CI->lproformainvoice->search_inovoice_item($customer_id);
		$this->template->full_admin_html_view($content);
	}
	//Manage invoice list
	   public function manage_proforma_invoice() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lproformainvoice');
        $CI->load->model('Invoices');
        $content = $this->lproformainvoice->invoice_list();
        $this->template->full_admin_html_view($content);
    }
    ...
    public function CheckInvoiceList(){
        ...
        $data = $this->Invoices->getProformaInvoiceList($postData);
        echo json_encode($data);
    } 
	...
	{	
        ...
        $content =$this->lproformainvoice->invoice_list_invoice_no($invoice_no);
		$this->template->full_admin_html_view($content);
	}
	...
	{	
        ...
        $content =$this->lproformainvoice->invoice_list_date_to_date($from_date,$to_date,$links,$config["per_page"],$page);
		$this->template->full_admin_html_view($content);
	}
	...
	public function pos_invoice(){
		...
		$content = $CI->lproformainvoice->pos_invoice_add_form();
		$this->template->full_admin_html_view($content);
	}
	...
	{	
		...
		$CI->auth->check_admin_auth();
		$CI->load->library('lproformainvoice');
		$content = $CI->lproformainvoice->invoice_html_data($invoice_id);
		$this->template->full_admin_html_view($content);
	}
	...
	{	
		...
		$CI->auth->check_admin_auth();
		$CI->load->library('lproformainvoice');
		$html_data = $CI->lproformainvoice->invoice_html_data_pdf($invoice_id);
		$CI->load->library('pdfgenerator');
        $pdf = $CI->pdfgenerator->generate($html_data, $invoice_id.'.pdf', TRUE, 'A4', "landscape");
	}
	...
	{	
      ...
      // $CI->permission1->method('invoice','read')->redirect();
		...
		$content = $CI->lproformainvoice->pos_invoice_html_data($invoice_id);		
		$this->template->full_admin_html_view($content);
	}
	...
	{	
	    if($this->session->userdata('user_type') == 4){
			...
			$product_info = $CI->Stockrequests->get_total_product_batch($batch_id);
			echo json_encode($product_info);
		}
		elseif($this->session->userdata('user_type') == 3){
            ...
            if($who_added==1) {
                ...
                $product_info = $CI->Stockrequests->get_total_product_batch_onss($batch_id, $mr_id);    
            }
	...
	{	
	    if($this->session->userdata('user_type') == 4){
			...
			$CI->load->model('Stockrequests');
			$product_id = $this->input->post('product_id');
			$product_info = $CI->Stockrequests->get_total_product_quantity($product_id);
			echo json_encode($product_info);
		}
		elseif($this->session->userdata('user_type') == 3){
			$CI =& get_instance();
	...
	public function invoice_delete($invoice_id)
	{
        ...
        if ($result) {
            ...
            return redirect("Cproformainvoice/manage_invoice");
        }

	}
	
	public function proforma_invoice_delete($invoice_id)
	{
	    //print_r($invoice_id);exit;
		$CI =& get_instance();
		$this->auth->check_admin_auth();
        ...
        if ($result) {
            $this->session->set_userdata(array('message'=>display('successfully_delete')));
            return redirect("Cproformainvoice/manage_proforma_invoice");
        }
    ...
    function uploadCsv_Invoice()
    {
        ...
        $this->session->set_userdata(array('message'=>display('successfully_added')));
        redirect(base_url('Cproformainvoice/manage_invoice'));
  ...
  // auto suggest customer
    ...
    public function previous() {
		...
		$CI->load->model('Customers');
		$customer_id = $this->input->post('customer_id');
		$sql = 'SELECT (SELECT SUM(amount) FROM customer_ledger WHERE d_c = "d" AND customer_id = "' . $customer_id . '") dr, (SELECT sum(amount) FROM customer_ledger WHERE d_c = "c" AND customer_id = "' . $customer_id . '") cr';
    ...
    public function instant_customer(){
        ...
        if($result == TRUE) {
        	...
        	$coa = $this->Customers->headcode();
           	if($coa->HeadCode!=NULL)	$headcode	=	$coa->HeadCode+1;
			else $headcode="102030001";
	...
	public function getitemlist(){
		...
		if(!empty($getproduct)){
			...
			$this->load->view('proformainvoice/getproductlist', $data);  
		}
		else{
			echo 420;
		}
	...
	public function convertIntoInvoice(){
		$p_id = $this->input->post('invoice_id');
		
		$CI =& get_instance();
		$CI->auth->check_admin_auth();
		$CI->load->model('Invoices');

		$invoice_id = $CI->Invoices->convertProformaToInvoice($p_id);
		
		$data = array();
		if($invoice_id>0){
			$data['status'] 	= true;
			$data['invoice_id'] = $invoice_id;
			$this->session->set_userdata(array('message'=>'Proforma Successfully converted into Invoice'));
		}
		else{
			$data['status'] 	= false;
			$data['invoice_id'] = $invoice_id;
