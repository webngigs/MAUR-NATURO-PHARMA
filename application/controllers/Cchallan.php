...
class Cchallan extends CI_Controller {
	...
	public function index()
	{
		...
		$CI->load->library('lchallan');
		$content = $CI->lchallan->invoice_add_form();
	...
	public function insert_invoice()
	{
		$CI =& get_instance();
		$CI->auth->check_admin_auth();
		$CI->load->model('Challans');
		 // $CI->permission1->method('invoice','create')->redirect();
		$invoice_id = $CI->Challans->invoice_entry();
		$this->session->set_userdata(array('message'=>display('successfully_added')));
		//$this->invoice_inserted_data($invoice_id);
		redirect("Cchallan/invoice_inserted_data/".$invoice_id);
	}
	 ...
	 public function manual_sales_insert(){
        ...
        if(!empty($invoice_id)){
            ...
            $data['details'] = $this->load->view('challan/invoice_html', $data, true);
    ...
    public function invoice_inserted_data_manual() {
        ...
        $content = $CI->lchallan->invoice_html_data_manual($invoice_id);
        $this->template->full_admin_html_view($content);
    }
    public function pos_invoice_inserted_data_manual() {
       $CI = & get_instance();
        ...
        $CI->load->library('lchallan');
        $invoice_id = $this->input->post('invoice_id');
        $content = $CI->lchallan->pos_invoice_html_data_manual($invoice_id);
        $this->template->full_admin_html_view($content);
    }
	...
	{	
		...
		$content = $CI->lchallan->invoice_edit_data($invoice_id);
		$this->template->full_admin_html_view($content);
	}
	// invoice Update
	public function invoice_update()
	{
		...
		$CI->load->model('Challans');
		//$CI->permission1->method('invoice','update')->redirect();
		$invoice_id = $CI->Challans->update_invoice();
	...
	public function search_inovoice_item()
	{
        ...
        $content = $CI->lchallan->search_inovoice_item($customer_id);
		$this->template->full_admin_html_view($content);
	}
	//Manage invoice list
	public function manage_challan() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lchallan');
        $CI->load->model('Challans');
        $content = $this->lchallan->invoice_list();
        $this->template->full_admin_html_view($content);
    }

	public function customer_challan() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lchallan');
        $CI->load->model('Challans');
	...
	{	
        ...
        $content =$this->lchallan->invoice_list_invoice_no($invoice_no);
		$this->template->full_admin_html_view($content);
	}
	...
	{	
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Cchallan/date_to_date_challan/');
        ...
        $content =$this->lchallan->invoice_list_date_to_date($from_date,$to_date,$links,$config["per_page"],$page);
		$this->template->full_admin_html_view($content);
	}
	...
	public function pos_invoice(){
		...
		$content = $CI->lchallan->pos_invoice_add_form();
		$this->template->full_admin_html_view($content);
	}
	...
	public function insert_pos_invoice()
	{
	   ...
	   // $CI->permission1->method('invoice','create')->redirect();
		...
		$product_details = $CI->Challans->pos_invoice_setup($product_id);
	...
	public function autocompleteproductsearch(){
        ...
        $product_info 	= $CI->Challans->autocompletproductdata($product_name);
	...
	{	
		...
		$CI->auth->check_admin_auth();
		$CI->load->library('lchallan');
		$content = $CI->lchallan->invoice_html_data($invoice_id);		
		$this->template->full_admin_html_view($content);
	}
	...
	{	
      ...
      // $CI->permission1->method('invoice','read')->redirect();
		...
		$content = $CI->lchallan->pos_invoice_html_data($invoice_id);		
		$this->template->full_admin_html_view($content);
	}
	...
	{	
		...
		$product_info = $CI->Challans->get_total_product($product_id,$manufacturer_id);
	...
	{	
		...
		elseif($this->session->userdata('user_type') == 3){
    		...
    		$CI->load->model('Challans');
    		$batch_id = $this->input->post('batch_id');
    		$product_info = $CI->Challans->get_total_product_batch($batch_id);
    		echo json_encode($product_info);
		}
		else{
    		$CI =& get_instance();
	...
	{	
		...
		elseif($this->session->userdata('user_type') == 3){
			...
			$CI->load->model('Challans');
			$product_id = $this->input->post('product_id');
			$product_info = $CI->Challans->get_total_product_invoic($product_id);
			echo json_encode($product_info);
		}
		else{
			$CI =& get_instance();
	...
	public function invoice_delete($invoice_id)
	{
		...
		$result = $CI->Challans->delete_invoice($invoice_id);
        if ($result) {
            $this->session->set_userdata(array('message'=>display('successfully_delete')));
            return redirect("Cchallan/manage_invoice");
        }
	...
	public function product_stock_check($product_id)
	{
		...
		$purchase_stocks = $CI->Challans->get_total_purchase_item($product_id);	
		$total_purchase = 0;		
		...
		$sales_stocks = $CI->Challans->get_total_sales_item($product_id);
		$total_sales = 0;	
    ...
    function uploadCsv_Invoice()
    {
        ...
        $this->session->set_userdata(array('message'=>display('successfully_added')));
        redirect(base_url('Cchallan/manage_invoice'));
  ...
  // auto suggest customer
    ...
    public function gui_pos(){
		...
		$content  = $this->parser->parse('challan/gui_pos_invoice', $data, true);
		$this->template->full_admin_html_view($content);  
    }
	...
	public function getitemlist(){
		...
		if(!empty($getproduct)){
			...
			$this->load->view('challan/getproductlist', $data);  
		}
		else{
			echo 420;
		}
