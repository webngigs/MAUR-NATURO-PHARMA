...
class Mrs extends CI_Model {
	...
	public function total_payment_received_from_client(){
		if($this->session->userdata('user_type') == 3){
			$result_array = $this->___getMrIdByLoggedUserId();
    		...
    		if(isset($query[0]->total_payment))    return $query[0]->total_payment;
	        else return 0;   
	    }
	...
	public function total_payment_received_from_mr(){
		if($this->session->userdata('user_type') == 1){
			$this->db->select('SUM(amount) AS total_mrpayment');
			$this->db->from('mrpaymments');
			$query = $this->db->get()->result();
			if(isset($query[0]->total_mrpayment))    return $query[0]->total_mrpayment;
			else return 0;   
		}
	...
	public function total_expenses_of_mr($expenses_status){
		if($this->session->userdata('user_type') == 1){
			$this->db->select('SUM(amount) AS total_amount');
			$this->db->from('expenses');
			$this->db->where('expenses_status', $expenses_status);
			$query = $this->db->get()->result();
			if(isset($query[0]->total_amount))    return $query[0]->total_amount;
			else return 0;	
		}
		else if($this->session->userdata('user_type') == 3){
			$result_array = $this->___getMrIdByLoggedUserId();
	...
	public function total_salary_of_mr(){
		if($this->session->userdata('user_type') == 1){
			$this->db->select('SUM(amount) AS total_amount');
