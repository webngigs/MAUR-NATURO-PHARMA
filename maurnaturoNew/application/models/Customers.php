...
class Customers extends CI_Model {
	...
	public function count_customer()
	{
		if($this->session->userdata('user_type') == 3)
		    return $this->db->where('user_id', $this->session->userdata('user_id'))->from("customer_information")->count_all_results();
		else
	...
	public function getCustomerList($postData=null){
		...
		## Fetch records
		$this->db->select("*, (SELECT mr_information.mr_name FROM mr_information WHERE customer_information.user_id=mr_information.user_id) AS mr_name");
		$this->db->from('customer_information');
		if($this->session->userdata('user_type') == 3)
			$this->db->where('user_id', $this->session->userdata('user_id'));
		if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->session->userdata('user_type') == 1){
                $button .=' <a href="'.$base_url.'Ccustomer/customer_change_password/'.$record->customer_id.'" class="btn btn-warning" data-placement="left" title="Change Customer Password"><i class="fa fa-key"></i></a>';
                
				if($this->permission1->method('manage_customer','update')->access()){
					$button .=' <a href="'.$base_url.'Ccustomer/customer_update_form/'.$record->customer_id.'" class="btn btn-info" data-placement="left" title="'.display('update').'"><i class="fa fa-edit"></i></a>';
				}
				$button .=' <a href="'.$base_url.'Ccustomer/customer_view/'.$record->customer_id.'" class="btn btn-success" data-placement="left" title="Customer Details"><i class="fa fa-eye"></i></a>';
				if($this->permission1->method('manage_customer','delete')->access()){
					$button .=' <a href="'.$base_url.'Ccustomer/customer_delete/'.$record->customer_id.'" class="btn btn-danger" onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
				}
    		}
    		else{
    		    $button .=' <a href="'.$base_url.'Ccustomer/customer_view/'.$record->customer_id.'" class="btn btn-success" data-placement="left" title="Customer Details"><i class="fa fa-eye"></i></a>';    
    		...
    		$this->db->where('user_id', $record->user_ref_id);
    		$recordsss = $this->db->get()->result();
    		$custStatus = $recordsss[0]->status;
    		
    		if($this->session->userdata('user_type') == 1){
        		$checki = '';
        		if($custStatus == 1)    $checki = 'checked="checked"';    
    			$checkinput = '<input type="checkbox" class="form-control" '.$checki.' onclick="ActiveOrInactiveCustomer(this, '.$record->user_ref_id.');"/>';
    		}
    		else{
    		    if($custStatus == 1)    $checkinput = '<span class="text-success">Active</span>';
    		    else                    $checkinput = '<span class="text-danger">Inctive</span>';
    		}
    		
            if(empty($record->mr_name)) $record->mr_name = '--';
            if(empty($record->customer_address)) $record->customer_address = '--';
            if(empty($record->customer_name)) $record->customer_name = '--';
            if(empty($record->customer_mobile)) $record->customer_mobile = '--';
            if(empty($record->gst_no)) $record->gst_no = '--';
    ...
    public function customer_search_by_term($term){
        if($this->session->userdata('user_type') == 3){
