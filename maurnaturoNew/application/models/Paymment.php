...
class Paymment extends CI_Model {
	public function __construct(){
		parent::__construct();
	}

	public function count_payment() {
		return $this->db->count_all("paymments");
	}
	...
	public function getPaymentList($postData=null){
        ...
        foreach($records as $record ){
			...
			$button	.=	'<div class="btn-group"><button type="button" class="btn btn-success " data-toggle="dropdown">Action <span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button><ul class="dropdown-menu" role="menu">';
				if($this->permission1->method('manage_payment','update')->access()){
				...
				if($this->permission1->method('manage_payment','delete')->access()){
					$button .=' <li class="action"><a href="'.$base_url.'Cpayment/payment_delete/'.$record->id.'" class="btn btn-danger " onclick="'.$jsaction.'"><i class="fa fa-trash"></i>DELETE</a></li>';
				}
		  	$button .='</ul></div>';
