...
class Lchallan {	
	//Retrieve  Invoice List
 	public function invoice_list() {
        ...
        $company_info = $CI->Challans->retrieve_company();
        $data = array(
            'title'         => display('manage_challan'),
            'total_invoice' => $CI->Challans->count_invoice(),
            'company_info'  => $company_info,
        );
        $invoiceList = $CI->parser->parse('challan/invoice', $data, true);
        return $invoiceList;
    }
	...
	public function invoice_list_invoice_no($invoice_no)
	{
		...
		$invoices_list = $CI->Challans->invoice_list_invoice_id($invoice_no);
	...
	public function invoice_list_date_to_date($from_date,$to_date,$links,$perpage,$page)
	{
		...
		$invoices_list = $CI->Challans->invoice_list_date_to_date($from_date,$to_date,$perpage,$page);
		if(!empty($invoices_list)){
			foreach($invoices_list as $k=>$v){
				$invoices_list[$k]['final_date'] = $CI->occational->dateConvert($invoices_list[$k]['date']);
				$invoices_list[$k]['payment_type'] = ($invoices_list[$k]['payment_type'] == 1?'Cash Payment':'Bank Payment');
	...
	public function pos_invoice_add_form()
	{
		...
		$CI->load->model('Challans');
		$CI->load->model('Web_settings');
		$customer_details = $CI->Challans->pos_customer_setup();
		$currency_details = $CI->Web_settings->retrieve_setting_editdata();
		...
		$invoiceForm = $CI->parser->parse('challan/add_pos_invoice_form',$data,true);
		return $invoiceForm;
	}
	...
	public function search_inovoice_item($customer_id)
	{
		...
		$CI->load->model('Challans');
		$CI->load->library('occational');
		$invoices_list = $CI->Challans->search_inovoice_item($customer_id);
		if(!empty($invoices_list)){
			foreach($invoices_list as $k=>$v){
				$invoices_list[$k]['final_date'] = $CI->occational->dateConvert($invoices_list[$k]['date']);
			}
	...
	public function invoice_add_form()
	{
		...
		$invoiceForm = $CI->parser->parse('challan/add_invoice_form',$data,true);
		return $invoiceForm;
	}
	...
	public function insert_invoice($data)
	{
		$CI =& get_instance();
		$CI->load->model('Challans');
        $CI->Challans->invoice_entry($data);
		return true;
	}
	...
	public function invoice_edit_data($invoice_id)
	{
		...
		$CI->load->model('Challans');
		$CI->load->model('Web_settings');
		$invoice_detail = $CI->Challans->retrieve_invoice_editdata($invoice_id);
		$bank_list      = $CI->Web_settings->bank_list();
		$taxinfo        = $CI->Challans->service_invoice_taxinfo($invoice_id);
        $taxfield       = $CI->db->select('tax_name,default_value')
			->from('tax_settings')
			->get()
			->result_array();
		...
		$chapterList = $CI->parser->parse('challan/edit_invoice_form',$data,true);
		return $chapterList;
	}
	...
	public function invoice_html_data($invoice_id)
	{
		...
		$CI->load->model('Web_settings');
		$CI->load->library('occational');
		$invoice_detail = $CI->Challans->retrieve_invoice_html_data($invoice_id);
		...
		$chapterList = $CI->parser->parse('challan/invoice_html',$data,true);
		return $chapterList;
	}
	...
	public function invoice_html_data_manual($invoice_id) {
		...
		$chapterList = $CI->parser->parse('challan/invoice_html_manual',$data,true);
		return $chapterList;
	 }
	...
	public function pos_invoice_html_data($invoice_id)
	{
		...
		$chapterList = $CI->parser->parse('challan/pos_invoice_html',$data,true);
		return $chapterList;
	}
	...
	public function pos_invoice_html_data_manual($invoice_id) {
		...
		$chapterList = $CI->parser->parse('challan/pos_invoice_html_direct',$data,true);
		return $chapterList;
