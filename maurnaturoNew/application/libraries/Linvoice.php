...
public function invoice_edit_data($invoice_id)
	...
	public function invoice_html_data($invoice_id)
	{
		...
		$customer_information = $CI->db->select('state_code')
                ->from('customer_information')
                ->where('customer_id', $invoice_detail[0]['customer_id'])
                ->get()
                ->result_array();
                
        $admin_information = $CI->db->select('state_code')
                ->from('users')
                ->where('user_id', 1)
                ->get()
                ->result_array();
		...
		$data=array(
			...
			'discount_type'  	=> $currency_details[0]['discount_type'],
			'tax_regno'         => $txregname,
			'customer_state_code'   =>  $customer_information[0]['state_code'],
	...
	public function invoice_html_data_pdf($invoice_id)
	{
		...
		$chapterList = $CI->parser->parse('invoice/invoice_html_pdf',$data,true);
		return $chapterList;
	}
