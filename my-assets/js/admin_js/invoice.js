...
function addInputField(t) {
    ...
    var cust_discout = $("#cust_discout").val();
    var count = row + 1;
    var limits = 500;
...
function quantity_calculate(item) {
    ...
    var taxnumber           =   $("#txfieldnum").val();
    var item_rate           =   $("#item_rate_" + item).html();
    ...
    if (quantity > 0 || discount > 0) {
        if (dis_type == 1) {
            ...
            var tmp_item_rate = quantity * item_rate;
            var dis_tmp_item_rate = (tmp_item_rate * discount / 100);
            var temp_total_item_rate = Math.round(tmp_item_rate - dis_tmp_item_rate);
            $("#total_item_rate_" + item).html(temp_total_item_rate);
        ...
        var tmp_item_rate = quantity * item_rate;
        $("#total_item_rate_" + item).html(tmp_item_rate);
    }
...
function calculateSum() {
    ...
    var taxnumber = $("#txfieldnum").val();
    var t = 0, a = 0, e = 0, o = 0, f = 0, p = 0, ad = 0, invdis =  $("#invdcount").val();
    //Total Tax
    for(var i=0;i<taxnumber;i++){
        var j = 0;
    ...
    var total_s = t;
    if(!isNaN(invdis))  total_s = total_s - invdis;
    $("#total_s").val(total_s.toFixed(2,2));
    
    var ik = 1;
    $(".tax_percentage").each(function () {
        if(!isNaN(this.value)){
            var total_price_amt = $("#total_price_"+ik).val();
