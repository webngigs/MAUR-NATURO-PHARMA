function addInputField(t) {
    var row     =   $("#normalstockrequest tbody tr").length;
    var count   =   row + 1;
    var limits  =   500;
    if(count==limits)   alert("You have reached the limit of adding "+count+" inputs");
        ...
        innerHTMLData += "</td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<select class='form-control' required id='batch_id_"+count+"' name='batch_id[]' onchange='product_stock("+count+")' tabindex='"+tab2+"'>";
                innerHTMLData += "<option></option>";
            innerHTMLData += "</select>";
        innerHTMLData += "</td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<input type='text' name='available_quantity[]' id='available_quantity_"+count+"' class='form-control text-right available_quantity_"+count+"' value='0' readonly='readonly' />";
        innerHTMLData += "</td>";
        innerHTMLData += "<td id='expire_date_"+count+"'></td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<input class='form-control text-right unit_"+count+" valid' value='None' readonly='' aria-invalid='false' type='text'/>";
        innerHTMLData += "</td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<input type='text' name='product_quantity[]' onkeyup='quantity_calculate("+count+"), checkqty("+count+");' onchange='quantity_calculate("+count+");' id='total_qntt_"+count+"' class='total_qntt_"+count+" form-control text-right' placeholder='0.00' min='0' tabindex='"+tab3+"' required/>";
        innerHTMLData += "</td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<input type='text' name='product_rate[]' onkeyup='quantity_calculate("+count+"), checkqty("+count+");' onchange='quantity_calculate("+count+");' id='price_item_"+count+"' class='price_item"+count+" form-control text-right' required placeholder='0.00' readonly min='0' tabindex='"+tab4+"'/>";
        innerHTMLData += "</td>";
        innerHTMLData += "<td class='text-right'>";
            innerHTMLData += "<input class='total_price form-control text-right' type='text' name='total_price[]' id='total_price_"+count+"' value='0.00' readonly='readonly'/>";
        innerHTMLData += "</td>";
        innerHTMLData += "<td>";
            innerHTMLData += "<a tabindex='"+tab6+"' style='text-align: right;' class='btn btn-danger' value='Delete' onclick='deleteRow(this)'><i class='fa fa-close'></i></a>";
        innerHTMLData += "</td>"; 
        ...
        document.getElementById("add_stockrequests_item").setAttribute("tabindex", tab7);
        document.getElementById("add_stockrequests").setAttribute("tabindex", tab11);
        count++;
    }
}
...
function stockLimit(t) {
    ...
    $.ajax({
        type    :   "POST",
        url     :   o + "Cstockrequest/product_stock_check",
        data    :   { product_id: e },
        cache   :   !1,
        success :   function(e) {
            if(a>Number(e)){
...
function deleteRow(t) {
    var a = $("#normalstockrequest > tbody > tr").length;
